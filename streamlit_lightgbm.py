...
st.set_page_config(page_title="LightGBM Model Trainer", page_icon="🌟", layout="wide")
...
if st.button("Load Example Dataset"):
    with st.spinner('Loading dataset...'):
        try:
            ...
            if dataset is None:
                raise ValueError("Failed to load dataset. The dataset returned None. Please check the dataset ID and try again.")
            ...
            print("Dataset loaded: X_train shape:", X_train.shape, "y_train shape:", y_train.shape)
        except Exception as e:
            st.error(f"Error loading dataset: {e}. Please verify your internet connection or try a different dataset.")
...
if uploaded_file is not None or sample_dataset_loaded:
    ...
    st.checkbox("Show full dataset", value=st.session_state.show_full_dataset, on_change=update_preview)
    print("Preview option selected:", st.session_state.show_full_dataset)  # Debug print statement
    ...
    if len(data.columns) < 2:
        st.error("The dataset must contain at least two columns: one target and at least one feature.")
        st.stop()

    # Select target and features
    st.sidebar.header("Data Configuration")
    target_column = st.sidebar.selectbox("Select the target column", data.columns)
    feature_columns = st.sidebar.multiselect("Select the feature columns", data.columns, default=data.columns.drop(target_column))
    ...
    early_stopping_rounds = st.sidebar.slider("Early Stopping Rounds", 5, 50, 10)
    default_threshold = st.sidebar.slider("Default Classification Threshold", 0.0, 1.0, 0.5)
    importance_type = st.sidebar.selectbox("Select Feature Importance Type", ['split', 'gain'], index=0)
    print("Model parameters: learning_rate=", learning_rate, ", num_leaves=", num_leaves, ", n_estimators=", n_estimators, ", feature_fraction=", feature_fraction, ", early_stopping_rounds=", early_stopping_rounds)
    ...
    if st.button("Train Model"):
        ...
        if not sample_dataset_loaded:
            ...
            print("Split data: X_train shape:", X_train.shape, "X_test shape:", X_test.shape, "y_train shape:", y_train.shape, "y_test shape:", y_test.shape)

        # Check for NaN or missing values
        if X_train.isnull().values.any() or y_train.isnull().values.any() or X_test.isnull().values.any() or y_test.isnull().values.any():
            st.error("The dataset contains NaN or missing values. Please clean your data before training.")
        ...
        with st.spinner('Training the model...'):
            try:
                model = lgb.train(params, train_data, num_boost_round=n_estimators, valid_sets=[train_data, test_data], early_stopping_rounds=early_stopping_rounds, verbose_eval=10)
